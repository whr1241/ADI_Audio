...
volatile float Channel1;
volatile float Channel2;
volatile float Channel3;
volatile float Channel4;
...
struct Config_Table Config_array_ADC[16] = {
		 ...
		 {ADAU1979_REG_SAI_CTRL0		,	0x12},/*I2S 48kHz*/
        		...
        		if(CallbackCount==1)
        		        		{
        		        			 ...
        		        			 Channel1 = conv_float_by(*pGetADC1++,-31);
        		        			 Channel2 = conv_float_by(*pGetADC1++,-31);
        		        			 Channel3 = conv_float_by(*pGetADC1++,-31);
        		        			 Channel4 = conv_float_by(*pGetADC1++,-31);

        		//         			   Place your audio algorithm here

        		        		     *pGetDAC1++ = conv_fix_by(Channel1, 31);
        		        		     *pGetDAC1++ = conv_fix_by(Channel2, 31);
        		        		     *pGetDAC1++ = conv_fix_by(Channel3, 31);
        		        		     *pGetDAC1++ = conv_fix_by(Channel4, 31);
...
static int Sport_Init()
{
	...
	CHECK_RESULT(eResult);
	eResult = adi_sport_ConfigMC(hSPORTDev4ATx,1u,3u,0u,true);
	CHECK_RESULT(eResult);
	...
	CHECK_RESULT(eResult);
	eResult = adi_sport_ConfigMC(hSPORTDev4BRx,1u,3u,0u,true);
	CHECK_RESULT(eResult);
