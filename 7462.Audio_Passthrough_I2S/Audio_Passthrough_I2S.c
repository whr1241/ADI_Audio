...
ADI_CACHE_ALIGN static int   int_SP0ABuffer1_2[COUNT];
ADI_CACHE_ALIGN static int   int_SP0ABuffer2_2[COUNT];

ADI_CACHE_ALIGN static int   int_SP0ABuffer4_2[COUNT];
ADI_CACHE_ALIGN static int   int_SP0ABuffer5_2[COUNT];
...
volatile uint8_t CallbackCount_2 = 0;

volatile uint32_t TestCallbackCount = 0;
volatile uint32_t TestCallbackCount_2 = 0;
...
static void SPORTCallback_2(
    ...
    )
    {
        ...
        switch (nEvent)
        {
            /* CASE (buffer processed) */
            case ADI_SPORT_EVENT_RX_BUFFER_PROCESSED:
            		TestCallbackCount_2 +=1;
            		CallbackCount_2 +=1;
            		if(CallbackCount_2==1)
            		{
            		   for(i=0;i<COUNT;i++)
            		   {
            			   int_SP0ABuffer1_2[i]=int_SP0ABuffer4_2[i];/*Copy ADC buffer to DAC buffer */
            		   }
            		}

            		if(CallbackCount_2==2)
    				{
    				   for(i=0;i<COUNT;i++)
    				   {
    					   int_SP0ABuffer2_2[i]=int_SP0ABuffer5_2[i];/* Copy ADC buffer to DAC buffers */
    				   }
    				   CallbackCount_2=0;
    				}
...
static int SPU_init(void)
{
    ...
    /* Make SPORT 5B to generate secure transactions */
...
void SRU_Init(void)
{
	...
	*pREG_PADS0_DAI1_IE = BITM_PADS_DAI1_IE_VALUE;
    ...
    SRU2(DAI1_PB05_O,SPT5_ACLK_I); /*DAC clock to SPORT 5A*/
    SRU2(DAI1_PB05_O,SPT5_BCLK_I); /*DAC clock to SPORT 5B*/
    SRU2(LOW,DAI1_PBEN05_I);

    SRU2(DAI1_PB04_O,SPT5_AFS_I);  /*DAC FS to SPORT 5A*/
    SRU2(DAI1_PB04_O,SPT5_BFS_I);  /*DAC FS to SPORT 5B*/
    SRU2(LOW,DAI1_PBEN04_I);

    SRU2(SPT4_AD0_O,DAI1_PB01_I);  /* SPORT 4A to DAC*/
    SRU2(HIGH,DAI1_PBEN01_I);

    SRU2(SPT5_AD0_O,DAI1_PB02_I);  /* SPORT 5A to DAC*/
    SRU2(HIGH,DAI1_PBEN02_I);

    SRU2(DAI1_PB06_O,SPT4_BD0_I);  /* ADC to SPORT 4A */
    SRU2(LOW,DAI1_PBEN06_I);

    SRU2(DAI1_PB07_O,SPT5_BD0_I);  /* ADC to SPORT 5A */
...
static void PrepareDescriptors (void)
{
	...
	iDESC_LIST_1_SP5A.pStartAddr	= (int *)int_SP0ABuffer1_2;
	...
	iDESC_LIST_2_SP5A.pStartAddr	= (int *)int_SP0ABuffer2_2;
...
static int Sport_Init(void)
{
	...
	/* Register SPORT Callback functions */
	eResult = adi_sport_RegisterCallback(hSPORTDev4BRx,SPORTCallback,NULL);
	CHECK_RESULT(eResult);

	eResult = adi_sport_RegisterCallback(hSPORTDev5BRx,SPORTCallback_2,NULL);
	CHECK_RESULT(eResult);
